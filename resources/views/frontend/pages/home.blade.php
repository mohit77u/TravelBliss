    ...
    <section class="popular py-28">
        <div class="lg:w-80% xl:w-[70%] md:w-[90%] w-[95%] mx-auto">
            ...
            <div class="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-4 mt-10">
                @foreach ($popularDestinations->slice(0,3) as $destination )
                    <div class="single-destination group p-5 h-full rounded-xl cursor-pointer"> 
                        <a href="{{ route('single.destination.get', [$destination->id, $destination->slug]) }}">
                            <div class="single-destination-details relative h-full overflow-hidden rounded-xl">
                                <img src="{{ $destination->featured_image }}" alt="{{ $destination->name }}" class="min-h-[400px] max-h-[400px] rounded-xl h-full group-hover:scale-110 transition-all duration-500 w-full object-cover">
                                <div class="bottom-details transition-all duration-500 absolute bottom-4 left-0 w-full h-auto px-4">
                                    <div class="details bg-white/80 backdrop-blur-xs rounded-xl p-4">
                                        <h4 class="text-gray-dark uppercase font-bold text-2xl mb-3">{{ $destination->name }}</h4>
                                        <p class="text-gray-500 font-light tracking-wide text-sm leading-6 four-line-truncate">{{ $destination->description }}</p> 
